...
frappe.ui.form.on("Task Assigning Tool", {
  ...
  assign: function (frm) {
    ...
    if (!assignFrom || !assignTo) {
      frappe.msgprint(
        'Please select both "Assign From" and "Assign To" users.'
      );
      return;
    }
    ...
    frappe.call({
      method:
        "one_compliance.one_compliance.doctype.task_assigning_tool.task_assigning_tool.reassign_tasks",
      args: {
  ...
  compliance_categories: function (frm) {
    ...
    if (selectedCategory) {
      ...
      frappe.call({
        method:
          "one_compliance.one_compliance.doctype.task_assigning_tool.task_assigning_tool.get_compliance_executives",
        ...
        callback: function (response) {
          ...
          fetchEmployeeName(selectedEmployee, function (employeeName) {
            ...
            var isEmployeeInExecutives = response.message.some(function (
              executive
            ) {
  ...
  add_employee: function (frm) {
    ...
    if (selectedEmployee && selectedCategory) {
      ...
      frappe.call({
        method:
          "one_compliance.one_compliance.doctype.task_assigning_tool.task_assigning_tool.add_employee_to_compliance_executive",
        args: {
...
let set_filters = function (frm) {
  frm.set_query("assign_from", function () {
    return {
      query:
        "one_compliance.one_compliance.doctype.task_assigning_tool.task_assigning_tool.get_users_by_department",
      filters: {
  ...
  frm.set_query("compliance_categories", function () {
    return {
      query:
        "one_compliance.one_compliance.doctype.task_assigning_tool.task_assigning_tool.get_compliance_categories_for_user",
      filters: {
...
function prepare_task_reassign(frm) {
  if (frm.doc.assign_from) {
    frappe.call({
      method:
        "one_compliance.one_compliance.doctype.task_assigning_tool.task_assigning_tool.get_tasks_for_user",
      args: {
      ...
      callback: (r) => {
        frm.toggle_display(
          "task_reassigns",
          r && r.message && r.message.length > 0
        );
        ...
        if (r && r.message && r.message.length > 0) {
          ...
          frappe.msgprint(
            __("Currently, the user '{0}' has no tasks.", [frm.doc.assign_from])
          );
        }
      },
    });
...
function get_available_subcategories(frm) {
  if (frm.doc.compliance_categories) {
    frappe.call({
      method:
        "one_compliance.one_compliance.doctype.task_assigning_tool.task_assigning_tool.get_available_subcategories",
      args: {
...
function add_employee_to_subcategories(frm, selectedSubcategories) {
  frappe.call({
    method:
      "one_compliance.one_compliance.doctype.task_assigning_tool.task_assigning_tool.add_to_subcategories",
    args: {
      employee: frm.doc.employee,
      compliance_category: frm.doc.compliance_categories,
